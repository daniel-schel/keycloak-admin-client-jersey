...
public class CreatedResponseUtil {
    ...
    public static String getCreatedId(Response response) throws WebApplicationException {
        ...
        if(response.getMetadata().containsKey("location")){
            location = URI.create(response.getMetadata().get("location").get(0).toString());
        }

        Response.Status status = Response.Status.fromStatusCode(response.getStatus());
        if (!status.equals(Response.Status.CREATED)) {
            throw new WebApplicationException(new Throwable("Create method returned status " +
                    status.getReasonPhrase() + " (Code: " + status.getStatusCode() + "); " +
